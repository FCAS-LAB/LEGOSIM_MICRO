...
int main(int argc, char** argv) {
    ...
    int idX = atoi(argv[1]);
    int idY = atoi(argv[2]);
    int64_t *d_dataA, *d_dataB, *d_dataC, *d_dataA1, *d_dataB1;
    cudaMalloc((void**)&d_dataA, sizeof(int64_t) * Row_A * Col_A);
    cudaMalloc((void**)&d_dataB, sizeof(int64_t) * Row_B * Col_B);
    cudaMalloc((void**)&d_dataA1, sizeof(int64_t) * Row_A * Col_A*1000);
    cudaMalloc((void**)&d_dataB1, sizeof(int64_t) * Row_B * Col_B*1000);
    cudaMalloc((void**)&d_dataC, sizeof(int64_t) * Col_B * Row_A);

    for(int i=0;i<100;i++){
        receiveMessage(idX, idY, 3, 3, d_dataA1, sizeof(int64_t) * Row_A * Col_A * 1000);
        receiveMessage(idX, idY, 3, 3, d_dataB1, sizeof(int64_t) * Row_B * Col_B * 1000);
